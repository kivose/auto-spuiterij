...
public class Outline : MonoBehaviour {
  ...
  private Color outlineColor = Color.white;
    ...
    [SerializeField]
    bool reverseGradientWhenOver;

    bool reversingGradient;

    float currentGradientTime;
  ...
  void OnEnable() {
    ...
    if(interactable != null)
        {
            interactable.selectEntered.AddListener(delegate { canShow = false; UpdateMaterialProperties(); });
            interactable.selectExited.AddListener(delegate { canShow = true; UpdateMaterialProperties(); });
        }
  }
  ...
  void Update() {
    if (needsUpdate) {
      ...
      UpdateMaterialProperties();
        ...
        if (useGradient)
        {
            currentGradientTime += Time.deltaTime * (reversingGradient? -1 : 1);
            if (reverseGradientWhenOver)
            {
                if(currentGradientTime >= gradientTime)
                {
                    reversingGradient = true;
                }
                else if(currentGradientTime <= 0)
                {
                    reversingGradient = false;
                }
            }
            else
            {
